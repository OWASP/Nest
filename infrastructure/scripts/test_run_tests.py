...
class TestRunTests(unittest.TestCase):
    ...
    def test_get_localstack_image_info_pro(self, mock_exists):
        ...
        dockerfile_content = "FROM localstack/localstack-pro:2026.6.0@sha256:123456\nUSER localstack\n"
        ...
        with patch("builtins.open", mock_open(read_data=dockerfile_content)):
            ...
            self.assertEqual(full_img, "localstack/localstack-pro:2026.6.0@sha256:123456")
    ...
    def test_cleanup(self, mock_sub_run, mock_remove, mock_exists):
        ...
        mock_sub_run.assert_any_call(["docker", "stop", run_tests.LOCALSTACK_CONTAINER_NAME], stdout=-3, stderr=-3)
        mock_sub_run.assert_any_call(["docker", "rm", run_tests.LOCALSTACK_CONTAINER_NAME], stdout=-3, stderr=-3)
    ...
    def test_start_localstack_success(self, mock_license, mock_healthy, mock_sleep, mock_sub_run, mock_env):
        ...
        mock_sub_run.assert_any_call([
            "docker", "run", "-d",
            "--name", run_tests.LOCALSTACK_CONTAINER_NAME,
            "-p", "4566:4566",
            "-e", "LOCALSTACK_AUTH_TOKEN=dummy-token",
    ...
    @patch("os.listdir")
    @patch("subprocess.run")
    def test_discover_and_run_tests_unit(self, mock_sub_run, mock_listdir, mock_walk, mock_exists):
        mock_exists.return_value = True
        mock_walk.return_value = [
            ("infrastructure/modules/storage/tests", [], ["integration.tftest.hcl", "storage.tftest.hcl"])
        ]
        ...
        mock_sub_run.assert_any_call(["terraform", "-chdir=infrastructure/modules/storage", "init", "-backend=false", "-input=false"])
        # In unit mode, it should filter storage.tftest.hcl (and NOT integration.tftest.hcl)
        mock_sub_run.assert_any_call(["terraform", "-chdir=infrastructure/modules/storage", "test", "-filter=tests/storage.tftest.hcl"])
