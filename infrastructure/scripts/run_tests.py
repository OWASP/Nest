...
def get_localstack_image_info(root_dir):
    ...
    if not match_line:
        print(f"Error: could not determine LocalStack image from {dockerfile_path}.", file=sys.stderr)
        sys.exit(1)
        
    full_image = re.sub(r"^FROM\s+", "", match_line).strip()
    
    match_tag = re.search(r"^FROM\s+localstack/localstack(?:-pro)?:\s*([^\s@]+)", match_line)
    if not match_tag:
        print(f"Error: could not determine LocalStack image tag from {dockerfile_path}.", file=sys.stderr)
        sys.exit(1)
...
def check_existing_overrides():
    for filepath, _ in OVERRIDES:
        if os.path.exists(filepath):
            print(f"Error: {filepath} already exists. Refusing to run to avoid overwriting.", file=sys.stderr)
            sys.exit(1)
...
def cleanup(localstack_started):
    ...
    if localstack_started:
        ...
        subprocess.run(["docker", "stop", LOCALSTACK_CONTAINER_NAME], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["docker", "rm", LOCALSTACK_CONTAINER_NAME], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def start_localstack(localstack_image):
    auth_token = os.environ.get("LOCALSTACK_AUTH_TOKEN")
    if not auth_token:
        print("Error: LOCALSTACK_AUTH_TOKEN environment variable is not set.", file=sys.stderr)
        print("LocalStack integration tests require a valid auth token to run.", file=sys.stderr)
        sys.exit(1)
        
    subprocess.run(["docker", "rm", "-f", LOCALSTACK_CONTAINER_NAME], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    for attempt in range(1, HEALTH_MAX_ATTEMPTS + 1):
        ...
        if attempt == HEALTH_MAX_ATTEMPTS:
            print(f"Error: LocalStack failed to become healthy within {HEALTH_MAX_ATTEMPTS * HEALTH_POLL_INTERVAL} seconds.", file=sys.stderr)
            sys.exit(1)
    ...
    for attempt in range(1, license_max_attempts + 1):
        ...
        if attempt == license_max_attempts:
            print(f"Error: LocalStack license failed to activate within {license_max_attempts * HEALTH_POLL_INTERVAL} seconds.", file=sys.stderr)
            subprocess.run(f"docker logs {LOCALSTACK_CONTAINER_NAME} 2>&1 | tail -30", shell=True)
            sys.exit(1)
        time.sleep(HEALTH_POLL_INTERVAL)
...
def discover_and_run_tests(mode):
    ...
    for test_dir in test_dirs:
        ...
        try:
            for entry in os.listdir(test_dir):
                if entry.endswith(".tftest.hcl"):
                    ...
                    if (mode == "integration" and is_integration) or (mode == "unit" and not is_integration):
        ...
        print(f"Testing {'integration' if mode == 'integration' else 'unit'} for {module_dir}...")
        
        init_cmd = ["terraform", f"-chdir={module_dir}", "init", "-backend=false", "-input=false"]
...
def main():
    ...
    group.add_argument("--get-tag", action="store_true", help="Print LocalStack image tag and exit")
    ...
    if args.integration:
        ...
        try:
            if localstack_healthy():
                ...
                print("LocalStack is not running on port 4566. Attempting to start it...")
