...
1. **IAM role `nest-staging-terraform` (lifecycle)**
   The staging pipeline assumes this role (see the "STSStateManagement" resource below). The role is created by the bootstrap workflow, which applies `infrastructure/bootstrap` (with `environment = "staging"`). Because the main CI/CD deployment pipeline runs image build, scan, and sign jobs (which assume this role) before running the bootstrap step, **the bootstrap workflow must be run first** (e.g. manually via `workflow_dispatch`) to create the role before the first full CI/CD deployment pipeline can run. Subsequent bootstrap runs update the staging role and policies.
