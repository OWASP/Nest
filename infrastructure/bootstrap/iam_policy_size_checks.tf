check "terraform_role_policy_sizes" {
  assert {
    condition = alltrue([
      for environment in local.environments : length(data.aws_iam_policy_document.part_one[environment].minified_json) <= 6144
    ])
  ...
  assert {
    condition = alltrue([
      for environment in local.environments : length(data.aws_iam_policy_document.part_two[environment].minified_json) <= 6144
    ])
