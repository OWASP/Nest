...
locals {
  external_runtime_secrets = merge({
    DJANGO_ALGOLIA_WRITE_API_KEY     = "Algolia write API key."
    DJANGO_OPEN_AI_SECRET_KEY        = "OpenAI API key."
    DJANGO_SENTRY_DSN                = "Django Sentry DSN."
    DJANGO_SLACK_BOT_TOKEN           = "Slack bot token."
    DJANGO_SLACK_SIGNING_SECRET      = "Slack signing secret."
    GITHUB_TOKEN                     = "GitHub API token."
    NEXT_SERVER_GITHUB_CLIENT_SECRET = "GitHub OAuth client secret."
    },
    var.enable_additional_parameters ? {
      NEST_GITHUB_APP_PRIVATE_KEY                     = "GitHub App private key."
      "SLACK_BOT_TOKEN_${var.slack_bot_token_suffix}" = "Slack workspace bot token."
    } : {}
  )
}

resource "aws_secretsmanager_secret" "external_runtime" {
  for_each = local.external_runtime_secrets
...
resource "aws_secretsmanager_secret_version" "django_secret_key" {
  ...
  secret_string = random_string.django_secret_key.result
}

resource "aws_secretsmanager_secret" "nextauth_secret" {
  description             = "NextAuth secret generated by Terraform."
  kms_key_id              = var.kms_key_arn
...
resource "aws_secretsmanager_secret_version" "nextauth_secret" {
  ...
  secret_string = random_string.nextauth_secret.result
}
...
resource "aws_ssm_parameter" "nest_github_app_private_key" {
  count       = var.enable_additional_parameters && var.runtime_secrets_mode == "prepare" ? 1 : 0
...
moved {
  from = aws_ssm_parameter.django_algolia_write_api_key
  to   = aws_ssm_parameter.django_algolia_write_api_key[0]
}

moved {
  from = aws_ssm_parameter.django_open_ai_secret_key
  to   = aws_ssm_parameter.django_open_ai_secret_key[0]
}

moved {
  from = aws_ssm_parameter.django_secret_key
  to   = aws_ssm_parameter.django_secret_key[0]
}

moved {
  from = aws_ssm_parameter.django_sentry_dsn
  to   = aws_ssm_parameter.django_sentry_dsn[0]
}

moved {
  from = aws_ssm_parameter.django_slack_bot_token
  to   = aws_ssm_parameter.django_slack_bot_token[0]
}

moved {
  from = aws_ssm_parameter.django_slack_signing_secret
  to   = aws_ssm_parameter.django_slack_signing_secret[0]
}

moved {
  from = aws_ssm_parameter.github_token
  to   = aws_ssm_parameter.github_token[0]
}

moved {
  from = aws_ssm_parameter.next_server_github_client_secret
  to   = aws_ssm_parameter.next_server_github_client_secret[0]
}

moved {
  from = aws_ssm_parameter.nextauth_secret
  to   = aws_ssm_parameter.nextauth_secret[0]
}
