...
resource "aws_ssm_parameter" "django_aws_media_bucket_name" {
  description = "The S3 bucket name for Django media files."
  name        = "/${var.project_name}/${var.environment}/DJANGO_AWS_MEDIA_BUCKET_NAME"
  tags        = var.common_tags
  type        = "String"
  value       = var.django_aws_media_bucket_name
}
