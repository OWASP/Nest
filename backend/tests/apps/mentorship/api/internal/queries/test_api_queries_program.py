...
class TestMyPrograms:
    ...
    def mock_info(self) -> MagicMock:
        """
        Create a MagicMock strawberry.Info whose context.request.user.github_user is a mocked GithubUser.
        
        Returns:
            MagicMock: A mock `strawberry.Info` object with `context.request.user` set to a mock user that has a `github_user` mock (id=1, login="testuser").
        """
    ...
    def test_my_programs_with_search(self, mock_program_prefetch: MagicMock, mock_mentor_select: MagicMock, mock_info: MagicMock) -> None:
        """
        Verify that my_programs applies a name search filter and returns matching programs.
        
        Asserts that the mentor is resolved, the Program queryset is filtered with name__icontains equal to the provided search string, and the resolver returns the expected program in the PaginatedPrograms result.
        """
        mock_mentor = MagicMock(spec=Mentor, id=1)
        mock_mentor_select.return_value.get.return_value = mock_mentor
    ...
    def test_my_programs_pagination(self, mock_program_prefetch: MagicMock, mock_mentor_select: MagicMock, mock_info: MagicMock) -> None:
        """
        Verify my_programs returns the correct page of programs and accurate pagination metadata when results span multiple pages.
        
        Asserts that requesting page 2 with a limit of 1 yields the program with id 2, total_pages equals 3, and current_page equals 2.
        """
        mock_mentor = MagicMock(spec=Mentor, id=1)
        mock_mentor_select.return_value.get.return_value = mock_mentor
