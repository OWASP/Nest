...
class TestGetMenteeDetails:
    ...
    def test_get_mentee_details_success(
        ...
        """
        Verify get_mentee_details returns a MenteeNode populated from Module, GithubUser, and Mentee lookups.
        
        Asserts the returned node contains the expected login, name, experience_level, and tags, and that
        Module.objects.only().get, GithubUser.objects.only().get, Mentee.objects.only().get, and
        MenteeModule.objects.filter(...).exists() are invoked with the expected arguments.
        """
...
class TestGetMenteeModuleIssues:
    ...
    def test_get_mentee_module_issues_pagination(
        ...
        """
        Verify that get_mentee_module_issues returns the correct slice of issues when pagination parameters are applied.
        
        Asserts the query returns the expected issues for the given limit and offset and that the issues queryset is filtered by the mentee's GitHub user (assignees).
        """
        mock_prefetch.return_value=MagicMock()
        mock_module = MagicMock(spec=Module, id=1)
        mock_module_only.return_value.get.return_value = mock_module
    ...
    def test_get_mentee_module_issues_pagination_out_of_range(
        ...
        """
        Verify that requesting issues with an offset beyond the available range returns an empty list.
        
        This test sets up a module, GitHub user, and mentee, ensures the mentee is enrolled in the module, configures the module's issues queryset to return no items for the requested slice, and asserts that get_mentee_module_issues(...) returns an empty list when offset is out of range.
        """
        mock_prefetch.return_value=MagicMock()
        mock_module = MagicMock(spec=Module, id=1)
        mock_module_only.return_value.get.return_value = mock_module
