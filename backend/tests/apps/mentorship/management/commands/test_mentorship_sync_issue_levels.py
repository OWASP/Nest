...
def test_handle_no_task_levels(MockTaskLevel, command):
    """When no TaskLevel objects exist, command should exit early with a warning."""
    
    empty_qs = make_qs([], exists=False)
    MockTaskLevel.objects.select_related.return_value.order_by.return_value = empty_qs
...
@patch("apps.mentorship.management.commands.mentorship_sync_issue_levels.TaskLevel")
def test_handle_updates_issues(MockTaskLevel, MockIssue, command):
    ...
    MockTaskLevel.objects.select_related.return_value.order_by.return_value = levels_qs

    label_a = MagicMock(); label_a.name = "Label-A"
    label_b = MagicMock(); label_b.name = "Label-B"
    label_nomatch = MagicMock(); label_nomatch.name = "No-Match"
    ...
    issues_qs = make_qs([issue_with_label_a, issue_with_label_b, issue_no_match, issue_already_up_to_date], exists=True)
    MockIssue.objects.prefetch_related.return_value.select_related.return_value = issues_qs
    ...
    MockIssue.objects.bulk_update.assert_called_once_with(expected_updated, ["level"])
    command.stdout.write.assert_any_call("Successfully updated the level for 3 issues.")


@patch("apps.mentorship.management.commands.mentorship_sync_issue_levels.Issue")
@patch("apps.mentorship.management.commands.mentorship_sync_issue_levels.TaskLevel")
def test_handle_no_updates_needed(MockTaskLevel, MockIssue, command):
    ...
    MockIssue.objects.bulk_update.assert_not_called()
