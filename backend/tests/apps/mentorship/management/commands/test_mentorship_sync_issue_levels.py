...
def command():
    """
    Create and return a Command instance prepared for tests with mocked output and styling.
    
    Returns:
        cmd (Command): A Command instance whose `stdout` is a MagicMock and whose `style` provides `WARNING` and `SUCCESS` callables that return their input.
    """
    cmd = Command()
...
def make_qs(iterable, exists=True):
    """
    Create a MagicMock that behaves like a queryset for tests.
    
    Parameters:
        iterable (iterable): Items to be yielded by iteration and returned by `.all()`.
        exists (bool): Value that `.exists()` will return.
    
    Returns:
        MagicMock: A mock queryset configured with `.exists()`, an iterator, and `.all()`.
    """
