...
def make_qs(iterable, exists=True):
    """
    Create a MagicMock that mimics a Django QuerySet for testing.
    
    Parameters:
        iterable (iterable): Items to populate the queryset iteration and .all() result.
        exists (bool): Value returned by the queryset's .exists() method.
    
    Returns:
        MagicMock: A mock object supporting iteration, .exists(), .count(), .distinct(), and .all() (which returns a list of the provided items).
    """
...
def make_user(user_id, login):
    """
    Create a MagicMock representing a user with `id` and `login` attributes set.
    
    Parameters:
        user_id: Value to assign to the mock's `id` attribute.
        login: Value to assign to the mock's `login` attribute.
    
    Returns:
        MagicMock: Mock user with `id` and `login` attributes set to the provided values.
    """
    user = MagicMock()
    user.id = user_id
    user.login = login
...
def make_comment(body, author, created_at=None):
    """
    Create a MagicMock representing a comment with minimal attributes used in tests.
    
    Parameters:
        body (str): The comment text.
        author (MagicMock): The comment author object (typically a mocked user).
        created_at (str|None): Value for the `nest_created_at` attribute; if None, defaults to "now".
    
    Returns:
        MagicMock: A mock comment with `body`, `author`, and `nest_created_at` attributes set.
    """
    c = MagicMock()
    c.body = body
    c.author = author
...
def command():
    """
    Create a Command instance configured for tests with mocked output and simplified style helpers.
    
    The returned Command has `stdout` replaced by a MagicMock and `style` replaced by a MagicMock whose
    WARNING, SUCCESS, and ERROR attributes are identity callables to make assertion of styled messages straightforward.
    
    Returns:
        Command: A Command instance with `stdout` set to a MagicMock and `style.WARNING`, `style.SUCCESS`,
        and `style.ERROR` set to identity lambdas.
    """
    cmd = Command()
...
def mock_module():
    """
    Create a MagicMock representing a module named "Test Module" whose project.repositories.filter().values_list().distinct() returns an iterable of repository IDs [1, 2].
    
    The returned mock's repository queryset also reports exists() as True (and supports iteration, count(), and .all()), matching expected behavior for tests that inspect published module repositories.
     
    Returns:
        MagicMock: A mock module with attribute `name` set to "Test Module" and a prepared repository queryset chain.
    """
    m = MagicMock()
    m.name = "Test Module"
...
def mock_issue():
    """
    Create a MagicMock representing an Issue with a preset number, title, and an empty comments queryset.
    
    The mock's comments queryset chain (.select_related().filter().order_by()) is configured to return an iterable queryset whose exists() is False.
    
    Returns:
        issue (MagicMock): Mock Issue with number=123, title="Test Issue Title", and an empty comments queryset.
    """
    issue = MagicMock()
    issue.number = 123
