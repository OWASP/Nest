...
class TestProjectLevelComplianceIntegration:
    ...
    def create_mock_metric(self, project):
        """Create a mock health metric for a project."""
    ...
    def test_complete_compliance_workflow_with_penalties(self, mock_get):
        """Test the complete workflow: fetch levels -> update projects -> calculate scores."""
        ...
        compliant_project = self.create_mock_project(
            "OWASP ZAP", "flagship", "lab"
        )  # Will be updated to flagship
        non_compliant_project = self.create_mock_project(
            "OWASP WebGoat", "lab", "other"
        )  # Will be updated to production
        missing_project = self.create_mock_project(
            "OWASP Missing", "lab", "lab"
        )  # Not in official data
        ...
        with (
            ...
            assert (
                missing_project.project_level_official == "lab"
            )  # Unchanged (not in official data)
        ...
        with (
            patch(METRICS_FILTER_PATCH) as mock_metrics_filter,
    ...
    def test_edge_cases_and_data_validation(self):
        ...
        extreme_requirements = self.create_mock_requirements(
            "lab", 999.0
