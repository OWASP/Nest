...
class TestProjectHealthMetricsQueryFiltering:
    ...
    def test_project_health_metrics_with_query(self, mock_get_latest_metrics):
        """Test project_health_metrics filters by query string."""
        base_qs = MagicMock()
        ...
        base_qs.filter.assert_called_once_with(project__name__icontains="security")
        assert result == filtered_qs

    @patch(
        "apps.owasp.models.project_health_metrics.ProjectHealthMetrics.get_latest_health_metrics"
    )
    def test_project_health_metrics_without_query(self, mock_get_latest_metrics):
        """Test project_health_metrics returns unfiltered queryset when query is empty."""
        base_qs = MagicMock()
        mock_get_latest_metrics.return_value = base_qs
    ...
    def test_project_health_metrics_distinct_length_with_query(self, mock_get_latest_metrics):
        """Test distinct_length filters by query string."""
        base_qs = MagicMock()
        filtered_qs = MagicMock()
        mock_get_latest_metrics.return_value = base_qs
        base_qs.filter.return_value = filtered_qs
        ...
        base_qs.filter.assert_called_once_with(project__name__icontains="owasp")
        assert result == 7
    ...
    def test_project_health_metrics_distinct_length_with_query_and_filters(
        ...
        """Test distinct_length with both query and filters using separate querysets."""
        base_qs = MagicMock()
        filtered_by_query_qs = MagicMock()
        filtered_by_both_qs = MagicMock()

        mock_get_latest_metrics.return_value = base_qs
        base_qs.filter.return_value = filtered_by_query_qs
        mock_apply.return_value = filtered_by_both_qs
        filtered_by_both_qs.count.return_value = 3

        mock_filters = MagicMock()
        query = ProjectHealthMetricsQuery()
        result = query.project_health_metrics_distinct_length(query="test", filters=mock_filters)

        base_qs.filter.assert_called_once_with(project__name__icontains="test")
        mock_apply.assert_called_once_with(mock_filters, filtered_by_query_qs)
        assert result == 3
