...
class TestUpdateReplicasCommand:
    ...
    def _setup(self):
        ...
        with (
            patch("apps.owasp.index.ProjectIndex.configure_replicas") as project_patch,
            patch("apps.owasp.index.ChapterIndex.configure_replicas") as chapter_patch,
        ):
            self.mock_project_update = project_patch
            self.mock_chapter_update = chapter_patch
            yield
    ...
    def test_successful_replica_configuration(self):
        ...
        with patch("sys.stdout", new=StringIO()) as fake_out:
            ...
            self.mock_chapter_update.assert_called_once()
            self.mock_project_update.assert_called_once()

    def test_handle_exception(self):
        """Test handling of exceptions during replica configuration."""
        error_message = "Failed to configure replicas"
        self.mock_chapter_update.side_effect = AlgoliaException(error_message)
        ...
        self.mock_chapter_update.assert_called_once()
        assert self.mock_project_update.called is False
