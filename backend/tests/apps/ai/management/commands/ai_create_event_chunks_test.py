...
class TestAiCreateEventChunksCommand:
    ...
    def test_command_help_text(self, command):
        ...
        assert command.help == "Create chunks for OWASP event data"
    ...
    @patch("apps.ai.management.commands.ai_create_event_chunks.openai.OpenAI")
    @patch("apps.ai.management.commands.ai_create_event_chunks.Event.upcoming_events")
    def test_handle_no_events_found(self, mock_upcoming_events, mock_openai, command):
    ...
    @patch.dict(os.environ, {"DJANGO_OPEN_AI_SECRET_KEY": "test-key"})
    @patch("apps.ai.management.commands.ai_create_event_chunks.openai.OpenAI")
    @patch("apps.ai.management.commands.ai_create_event_chunks.Event.objects")
    def test_handle_with_all_flag(self, mock_event_objects, mock_openai, command, mock_event):
    ...
    @patch("apps.ai.management.commands.ai_create_event_chunks.openai.OpenAI")
    @patch("apps.ai.management.commands.ai_create_event_chunks.Event.upcoming_events")
    def test_handle_default_future_events(
        self, mock_upcoming_events, mock_openai, command, mock_event
    ):
