...
class TestAiUpdateRepositoryContextCommand:
    ...
    def test_command_help_text(self, command):
        ...
        assert command.help() == "Update context for OWASP repository data"
    ...
    def test_extract_content(self, command, mock_repository):
        ...
        with patch(
            "apps.ai.management.commands.ai_update_repository_context.extract_repository_markdown_content"
        ) as mock_extract:
    ...
    def test_queryset_excludes_archived_repositories(self, command):
        """Test that the queryset excludes archived repositories."""
        with patch.object(command.__class__.__bases__[0], "get_base_queryset") as mock_super:
            mock_queryset = Mock()
            mock_super.return_value = mock_queryset
            mock_queryset.filter.return_value = "filtered_queryset"
    ...
    def test_queryset_excludes_empty_repositories(self, command):
        """Test that the queryset excludes empty repositories."""
        with patch.object(command.__class__.__bases__[0], "get_base_queryset") as mock_super:
            mock_queryset = Mock()
            mock_super.return_value = mock_queryset
            mock_queryset.filter.return_value = "filtered_queryset"
