...
class TestReleaseQuery:
    ...
    def test_recent_releases_with_login(self, mock_objects, mock_queryset, mock_release):
        ...
        # select_related is already called on objects, so filter should be on queryset
    ...
    def test_recent_releases_distinct_with_login(self, mock_objects, mock_queryset, mock_release):
        ...
        # One filter for initial filters, one for login, one for rank=1
    ...
    def test_recent_releases_distinct_with_organization(
        ...
        assert result == [mock_release]
        mock_queryset.annotate.assert_called_once()
        # One filter for initial filters, one for organization, one for rank=1
        assert mock_queryset.filter.call_count == 3
    ...
    def test_recent_releases_distinct_with_all_filters(
        ...
        # One filter for initial filters, one for login, one for organization, one for rank=1
        assert mock_queryset.filter.call_count == 4
