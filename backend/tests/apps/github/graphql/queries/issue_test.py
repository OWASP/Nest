...
class TestIssueQuery:
    ...
    def test_recent_issues_basic(self, mock_select_related, mock_info, mock_issue):
        ...
        result = IssueQuery().recent_issues(info=mock_info)
    ...
    @patch("apps.github.models.issue.Issue.objects.select_related")
    def test_recent_issues_with_login(self, mock_select_related, mock_info, mock_issue):
        """Test filtering issues by login."""
        mock_queryset = MagicMock()
        mock_queryset.order_by.return_value.filter.return_value = [mock_issue]
        mock_select_related.return_value = mock_queryset

        result = IssueQuery().recent_issues(info=mock_info, login="alice")
    ...
    @patch("apps.github.models.issue.Issue.objects.select_related")
    def test_recent_issues_with_organization(self, mock_select_related, mock_info, mock_issue):
        """Test filtering issues by organization."""
        mock_queryset = MagicMock()
        mock_queryset.order_by.return_value.filter.return_value = [mock_issue]
        mock_select_related.return_value = mock_queryset

        result = IssueQuery().recent_issues(info=mock_info, organization="orgX")
    ...
    @patch("apps.github.models.issue.Issue.objects.select_related")
    def test_recent_issues_limit(self, mock_select_related, mock_info, mock_issue):
        """Test limiting the number of issues returned."""
        mock_queryset = MagicMock()
        mock_queryset.order_by.return_value.__getitem__.return_value = [mock_issue]
        mock_select_related.return_value = mock_queryset

        result = IssueQuery().recent_issues(info=mock_info, limit=1)
