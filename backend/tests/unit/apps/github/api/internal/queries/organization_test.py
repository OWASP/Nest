...
class TestOrganizationQuery:
    ...
    @pytest.mark.asyncio
    async def test_organization_found(self, mock_aget, mock_organization):
        """Test fetching organization when it exists."""
        mock_aget.return_value = mock_organization

        result = await OrganizationQuery().organization(login="owasp")
    ...
    @patch("apps.github.models.organization.Organization.objects.aget", new_callable=AsyncMock)
    @pytest.mark.asyncio
    async def test_organization_not_found(self, mock_aget):
        """Test fetching organization when it doesn't exist."""
        mock_aget.side_effect = Organization.DoesNotExist()

        result = await OrganizationQuery().organization(login="nonexistent")
    ...
    @patch("apps.github.models.organization.Organization.objects.aget", new_callable=AsyncMock)
    @pytest.mark.asyncio
    async def test_organization_with_different_login(self, mock_aget, mock_organization):
        """Test fetching organization with different login."""
        mock_aget.return_value = mock_organization

        result = await OrganizationQuery().organization(login="test-org")
