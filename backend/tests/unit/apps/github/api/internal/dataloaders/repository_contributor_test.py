...
from apps.github.api.internal.dataloaders.repository_contributor import (
    ...
    load_top_contributors_by_committee_id,
...
class TestLoadTopContributorsByCommitteeId:
    """Tests for load_top_contributors_by_committee_id."""
    ...
    @staticmethod
    def _setup_committee_objects(mock_committee_objects):
        """Configure Committee.objects mock to return subquery objects."""
        mock_committee_repository_ids = MagicMock()
        mock_committee_id_subquery = MagicMock()
        mock_committee_objects.filter.return_value.values.side_effect = [
            mock_committee_repository_ids,
            mock_committee_id_subquery,
        ]
        return mock_committee_repository_ids, mock_committee_id_subquery

    @staticmethod
    def _ordered_qs(mock_qs):
        """Return the mock queryset at the end of the committee contributors chain."""
        filtered = mock_qs.filter.return_value
        annotated = filtered.annotate.return_value
        ranked = annotated.filter.return_value
        values = ranked.values.return_value
    ...
    @patch(
        "apps.github.api.internal.dataloaders.repository_contributor.Committee.objects",
    )
    @patch(
        "apps.github.api.internal.dataloaders.repository_contributor.get_top_contributors_by_keys",
        new_callable=AsyncMock,
    ...
    async def test_builds_queryset_with_correct_chain(
        ...
        mock_sync_to_async,
        mock_get_top_contributors,
        mock_committee_objects,
    ):
        """Queryset is built with filter, annotate, filter, values, order_by."""
        committee_ids = [1, 2, 3]
        mock_qs = MagicMock()
        self._setup_sync_to_async(mock_sync_to_async, mock_qs)
        mock_committee_repository_ids, _ = self._setup_committee_objects(mock_committee_objects)
        mock_get_top_contributors.return_value = [[], [], []]

        await load_top_contributors_by_committee_id(committee_ids)

        mock_qs.filter.assert_called_once_with(repository_id__in=mock_committee_repository_ids)
        mock_qs.filter.return_value.annotate.assert_called_once()
        mock_qs.filter.return_value.annotate.return_value.filter.assert_called_once()
        mock_qs.filter.return_value.annotate.return_value.filter.return_value.values.assert_called_once()
        mock_qs.filter.return_value.annotate.return_value.filter.return_value.values.return_value.order_by.assert_called_once_with(
            "committee_id", "-contributions_count"
        )
    ...
    async def test_delegates_to_get_top_contributors_by_keys(
        ...
        """Delegates to get_top_contributors_by_keys keyed by committee_id."""
        committee_ids = [10, 20]
        mock_qs = MagicMock()
        self._setup_sync_to_async(mock_sync_to_async, mock_qs)
        self._setup_committee_objects(mock_committee_objects)
        mock_get_top_contributors.return_value = [[], []]
    ...
    async def test_returns_result_from_get_top_contributors(
        ...
        result = await load_top_contributors_by_committee_id(committee_ids)

        assert result == expected

    @pytest.mark.asyncio
    async def test_empty_committee_ids(self):
        """An empty committee_ids list returns an empty list."""
        result = await load_top_contributors_by_committee_id([])
        assert result == []
...
class TestGetRepositoryContributorLoaders:
    ...
    def test_load_fn_is_load_top_contributors_by_committee_id(self):
        """The committee loader is wired to load_top_contributors_by_committee_id."""
        loaders = get_repository_contributor_loaders()
        loader = loaders[TOP_CONTRIBUTORS_BY_COMMITTEE_ID_LOADER]
        assert loader.load_fn is load_top_contributors_by_committee_id
