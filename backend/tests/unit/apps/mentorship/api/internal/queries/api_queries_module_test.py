...
class TestModuleQuery:
    ...
    async def test_get_module_success(
        ...
        mock_module_select_related.return_value.prefetch_related.return_value.aget = AsyncMock(
            return_value=mock_module
        )

        result = await api_module_queries.get_module(
            info=mock_info, module_key="module1", program_key="program1"
        )

        assert result == mock_module
        mock_module_select_related.assert_called_once_with("program", "project")
        mock_module_select_related.return_value.prefetch_related.return_value.aget.assert_called_once_with(
            key="module1", program__key="program1"
        )

    @patch("apps.mentorship.api.internal.queries.module.Module.objects.select_related")
    @pytest.mark.asyncio
    async def test_get_module_does_not_exist(
        self, mock_module_select_related: MagicMock, mock_info: MagicMock, api_module_queries
    ) -> None:
        """Test when the module does not exist."""
        mock_module_select_related.return_value.prefetch_related.return_value.aget = AsyncMock(
            side_effect=Module.DoesNotExist
        )
    ...
    @patch("apps.mentorship.api.internal.queries.module.Module.objects.select_related")
    @pytest.mark.asyncio
    async def test_get_module_hidden_for_draft_program(
        self,
    ...
    @patch("apps.mentorship.api.internal.queries.module.Module.objects.filter")
    @patch(
        "apps.mentorship.api.internal.queries.module.Program.objects.aget",
        new_callable=AsyncMock,
    )
    @pytest.mark.asyncio
    async def test_management_program_modules_admin_sees_all(
        self,
        mock_program_aget: AsyncMock,
        mock_module_filter: MagicMock,
        mock_info: MagicMock,
        ...
        result = await api_module_queries.get_management_program_modules(
            info=mock_info, program_key="program1"
        )
    ...
    @pytest.mark.asyncio
    async def test_management_program_modules_mentor_sees_only_assigned(
        self,
        mock_program_aget: AsyncMock,
        mock_module_filter: MagicMock,
        mock_info: MagicMock,
    ...
    @pytest.mark.asyncio
    async def test_management_program_modules_forbidden(
        self,
        mock_program_aget: AsyncMock,
        mock_info: MagicMock,
        api_module_queries,
        ...
        with pytest.raises(GraphQLError) as exc_info:
            await api_module_queries.get_management_program_modules(
                info=mock_info, program_key="program1"
            )
    ...
    @patch("apps.mentorship.api.internal.queries.module.Module.objects.select_related")
    @pytest.mark.asyncio
    async def test_management_module_admin_success(
        self, mock_module_select_related: MagicMock, mock_info: MagicMock, api_module_queries
    ) -> None:
        ...
        result = await api_module_queries.get_management_module(
            info=mock_info, module_key="module1", program_key="program1"
        )
    ...
    @patch("apps.mentorship.api.internal.queries.module.Module.objects.select_related")
    @pytest.mark.asyncio
    async def test_management_module_assigned_mentor_success(
        self,
        mock_module_select_related: MagicMock,
        mock_info: MagicMock,
        api_module_queries,
    ...
    @patch("apps.mentorship.api.internal.queries.module.Module.objects.select_related")
    @pytest.mark.asyncio
    async def test_management_module_enrolled_mentee_success(
        self,
        mock_module_select_related: MagicMock,
        mock_info: MagicMock,
        api_module_queries,
        ...
        mock_program = MagicMock(spec=Program)
        mock_program.has_admin = MagicMock(return_value=False)
        mock_module = MagicMock(spec=Module)
        mock_module.program = mock_program
        mock_module.has_mentor = MagicMock(return_value=False)
        mock_module.has_mentee = MagicMock(return_value=True)
        mock_module_select_related.return_value.prefetch_related.return_value.aget = AsyncMock(
            return_value=mock_module
        )
    ...
    @patch("apps.mentorship.api.internal.queries.module.Module.objects.select_related")
    @pytest.mark.asyncio
    async def test_management_module_unassigned_mentor_forbidden(
        self,
        mock_module_select_related: MagicMock,
        mock_info: MagicMock,
        api_module_queries,
        ...
        with pytest.raises(GraphQLError) as exc_info:
            await api_module_queries.get_management_module(
                info=mock_info, module_key="module1", program_key="program1"
            )
    ...
    @patch("apps.mentorship.api.internal.queries.module.Module.objects.select_related")
    @pytest.mark.asyncio
    async def test_management_module_not_found(
        self, mock_module_select_related: MagicMock, mock_info: MagicMock, api_module_queries
    ) -> None:
