...
class TestResultsByKeys:
    ...
    def test_basic_mapping(self):
        ...
        result = asyncio.run(results_by_keys(make_qs(items), [1, 2], "org_id", "repo"))
        assert result == [["repo-a"], ["repo-b"]]

    def test_empty_queryset(self):
        """Empty queryset returns an empty list for every key."""
        result = asyncio.run(results_by_keys(make_qs([]), [1, 2, 3], "org_id", "repo"))
        assert result == [[], [], []]

    def test_empty_keys(self):
        """Empty keys list returns an empty list regardless of queryset content."""
        items = [make_item(org_id=1, repo="repo-a")]
        result = asyncio.run(results_by_keys(make_qs(items), [], "org_id", "repo"))
        assert result == []

    def test_key_absent_from_results(self):
    ...
    def test_order_matches_keys_not_queryset(self):
        ...
        result = asyncio.run(results_by_keys(make_qs(items), [1, 2, 3], "org_id", "repo"))
        assert result == [["repo-a"], ["repo-b"], ["repo-c"]]

    def test_items_not_in_keys_are_ignored(self):
        """Items whose key is not present in ``keys`` are silently discarded."""
        items = [
            make_item(org_id=1, repo="repo-a"),
            make_item(org_id=99, repo="orphan"),  # key 99 is not requested
        ]
        result = asyncio.run(results_by_keys(make_qs(items), [1], "org_id", "repo"))
        assert result == [["repo-a"]]
    ...
    def test_arbitrary_field_names(self):
        ...
        result = asyncio.run(results_by_keys(make_qs(items), ["uk", "us"], "chapter_id", "city"))
        assert result == [["London"], ["New York", "San Francisco"]]

    def test_duplicate_keys_in_keys_list(self):
        """A key appearing multiple times in ``keys`` produces one entry per occurrence."""
        items = [make_item(org_id=1, repo="repo-a")]
        result = asyncio.run(results_by_keys(make_qs(items), [1, 1], "org_id", "repo"))
    ...
    def test_parametrized_scenarios(self, items, keys, key_field, value_field, expected):
        """Parametrized spot-checks covering single value, empty, and multi-value cases."""
        result = asyncio.run(results_by_keys(make_qs(items), keys, key_field, value_field))
