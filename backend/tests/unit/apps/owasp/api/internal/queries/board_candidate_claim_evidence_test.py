...
class TestBoardCandidateClaimEvidenceQuery:
    ...
    @patch("apps.owasp.api.internal.queries.board_candidate_claim_evidence.BoardCandidateClaim")
    def test_board_candidate_claim_evidences_non_self_rejected(self, mock_claim_model):
        mock_claim_model.Status = BoardCandidateClaim.Status
        user = MagicMock()
        user.is_authenticated = True
        user.github_user = MagicMock()
...
class TestBoardCandidateClaimEvidenceSingleQuery:
    ...
    def test_board_candidate_claim_evidence_non_self_rejected(self):
        user = MagicMock()
        user.is_authenticated = True
        user.github_user = MagicMock()
        info = _make_info(user)
        ...
        evidence.claim.board.reviewers.filter.return_value.exists.return_value = False
        evidence.claim.candidate.member = None
        evidence.claim.status = BoardCandidateClaim.Status.REJECTED
    ...
    def test_board_candidate_claim_evidence_reviewer_sees_rejected(self):
        user = MagicMock()
        user.is_authenticated = True
        user.github_user = MagicMock()
        info = _make_info(user)
...
class TestBoardCandidateClaimEvidenceFileUrlQuery:
    ...
    def test_file_url_anonymous_rejected(self):
        user = MagicMock()
        user.is_authenticated = False
        info = _make_info(user)
