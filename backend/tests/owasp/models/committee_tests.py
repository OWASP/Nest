...
def test_str_representation(name, expected_str):
...
def test_active_committees_count(value):
...
def test_save_method(has_summary):
    committee = Committee()
    committee.generate_summary = Mock()
    committee.summary = "Existing Summary" if has_summary else None
