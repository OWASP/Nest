    ...
    def handle(self, *args, **options):
        ...
        try:
            if level:
                ...
                print(
                    f"{action} health requirements for {requirements.get_level_display()} projects"
                )
            else:
                for level_choice in Project.ProjectLevel.choices:
                    ...
                    if created:
                        print(
                            f"Created default health requirements for {level_choice[1]} projects"
                        )
                    else:
                        print(f"Health requirements already exist for {level_choice[1]} projects")

        except Exception as e:
            print(f"Error updating health requirements: {e!s}")
            raise
