...
class Command(BaseCommand):
    ...
    def calculate_health_score(self, metrics, requirements):
        ...
        if metrics.last_released_at and (now - metrics.last_released_at).days <= requirements.last_release_days:
            score += weight

        if metrics.last_committed_at and (now - metrics.last_committed_at).days <= requirements.last_commit_days:
            score += weight
        ...
        if metrics.owasp_page_last_updated_at and (now - metrics.owasp_page_last_updated_at).days <= requirements.owasp_page_update_days:
            score += weight

        if metrics.pull_request_last_created_at and (now - metrics.pull_request_last_created_at).days <= requirements.last_pull_request_days:
            score += weight
    ...
    def get_pull_request_metrics(self, project):
        ...
        for repo in project.repositories.all():
            ...
            if repo_latest_pr and (not latest_pr or repo_latest_pr.created_at > latest_pr.created_at):
    ...
    def get_issue_metrics(self, project, requirements):
        ...
        return project.repositories.aggregate(
            ...
            unanswered_issues=Count('issues', 
                filter=Q(issues__comments_count=0, issues__state='open')),
            unassigned_issues=Count('issues', 
                filter=Q(issues__assignee__isnull=True, issues__state='open')),
            recent_releases=Count('releases', 
                filter=Q(releases__published_at__year__gte=window_start))
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f'Error updating metrics: {str(e)}'))
            raise