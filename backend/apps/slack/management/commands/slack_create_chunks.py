...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    if not openai_api_key:
      self.stdout.write(self.style.ERROR("DJANGO_OPEN_AI_SECRET_KEY environment variable not set"))
      return
  ...
  def create_chunks_from_message(self, message: Message) -> list[Chunk]:
    ...
    if not chunk_texts:
      self.stdout.write(f"No chunks created for message {message.slack_message_id} - text too short")
      return []
    ...
    try:
      ...
      self.stdout.write(f"Failed to create embeddings: {e}")
      return []
