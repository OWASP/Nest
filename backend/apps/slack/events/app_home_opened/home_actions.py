    ...
    try:
        if action_id == 'view_projects_action':
            ...
            if project_details['hits']:
                content = f"Here are the top {len(project_details['hits'])} projects you can explore:\n"
                    ...
                    project_card = {
                        ...
                        "text": {
                            ...
                            "text": (
                                ...
                                f"• :bust_in_silhouette: *Contributors:* {project['idx_contributors_count']}\n"
            ...
            if committee_details['hits']:
                content = f"Here are the top {len(committee_details['hits'])} committees you can join:\n"
            ...
            if chapter_details['hits']:
                content = f"Here are the top {len(chapter_details['hits'])} chapters you can be a part of:\n"
        ...
        logger.info(
            f"Home tab updated for user {user_id} after clicking button {action_id}")

    except SlackApiError as e:
        logger.error(
            f"Error updating Home Tab for user {user_id}: {e.response['error']}")
