...
export const fetchHeatmapData = async (username: string): Promise<HeatmapResponse | null> => {
  try {
    ...
    heatmapData.contributions = heatmapData.contributions.filter(
      (item: HeatmapContribution) => new Date(item.date) <= endDate && new Date(item.date) >= startDate
    )
    ...
    const transformedContributions: HeatmapContribution[] = allDates.map((date) => {
      const contribution = heatmapData.contributions.find((c: HeatmapContribution) => c.date === date)
      return contribution
        ? {
