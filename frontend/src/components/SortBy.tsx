    ...
    <div className="flex items-center gap-2">
      ...
      <div className="inline-flex h-12 items-center rounded-lg border border-gray-300 bg-gray-100 pl-3 shadow-sm transition-all duration-200 hover:shadow-md dark:border-gray-600 dark:bg-[#323232]">
        <Select
          ...
          classNames={{
            ...
            selectorIcon: 'text-gray-500 dark:text-gray-200 transition-transform duration-200',
            popoverContent:
        ...
        >
          {sortOptions.map((option: { label: string; key: string }) => (
            <SelectItem
              key={option.key}
              classNames={{
                base: 'text-sm text-gray-700 dark:text-gray-300 hover:bg-transparent dark:hover:bg-transparent focus:bg-gray-100 dark:focus:bg-[#404040] focus:outline-none rounded-sm px-3 py-2 cursor-pointer data-[selected=true]:bg-blue-50 dark:data-[selected=true]:bg-blue-900/20 data-[selected=true]:text-blue-600 dark:data-[selected=true]:text-blue-300 data-[focus=true]:bg-gray-100 dark:data-[focus=true]:bg-[#404040]',
              }}
            >
              {option.label}
            </SelectItem>
      ...
      {selectedSortOption !== 'default' && (
        ...
        >
          ...
          >
            {selectedOrder === 'asc' ? (
              <FaArrowUpWideShort className="h-4 w-4 text-gray-800 dark:text-gray-300" />
            ) : (
              <FaArrowDownWideShort className="h-4 w-4 text-gray-800 dark:text-gray-300" />
            )}
          </button>
        </Tooltip>
      )}
