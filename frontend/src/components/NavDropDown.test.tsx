...
describe('NavDropdown Component', () => {
  ...
  describe('Event Handling', () => {
    ...
    it('handles keyboard events on dropdown button', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('Getting Started')).not.to);
      });
    });
  ...
  describe('State Changes and Internal Logic', () => {
    ...
    it('manages dropdown state independently for multiple instances', async () => {
      const user = userEvent.setup();
      const secondLink: LinkType = {
        text: 'Second Menu',
        href: '/second',
        submenu: [{ text: 'Second Item', href: '/second/item' }],
      };
      
      render(
        <div>
          <NavDropdown {...defaultProps} />
          <NavDropdown pathname="/test" link={secondLink} />
        </div>
      );

      const firstButton = screen.getByRole('button', { name: /documentation/i });
      const secondButton = screen.getByRole('button', { name: /second menu/i });
      ...
      expect(screen.getByText('Getting Started')).toBeInTheDocument(); // First should still be open
    });
  });
  ...
  describe('DOM Structure and Styling', () => {
    ...
    it('applies active styling to submenu item when pathname matches', async () => {
      ...
      render(<NavDropdown {...propsWithActiveItem} />);

      const button = screen.getByRole('button');
      await user.click(button);
      
      const activeItem = screen.getByText('Getting Started').closest('a');
      expect(activeItem).toHaveClass('bg-slate-500', 'font-bold', 'text-white');
    });
    ...
    it('applies hover styling to inactive submenu items', async () => {
      ...
      const inactiveItem = screen.getByText('API Reference').closest('a');
