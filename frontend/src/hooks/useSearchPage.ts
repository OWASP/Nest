...
interface UseSearchPageOptions {
  ...
  hitsPerPage?: number
  useBackend?: SearchBackend // Use the proper type
}
...
export function useSearchPage<T>({
  ...
  defaultCategory = '',
  hitsPerPage = 25,
  useBackend = 'algolia', // Default to Algolia for backward compatibility
  ...
  useEffect(() => {
    if (useBackend !== 'algolia') return // Skip if using GraphQL
  ...
  // Use GraphQL data if backend is graphql
  useEffect(() => {
    if (useBackend !== 'graphql') return // Skip if using Algolia
