...
const About = () => {
  ...
  useEffect(() => {
    if (requestError) {
      handleAppError(requestError)
    }
  }, [requestError])

  // Build leadersData from the combined response
  const leadersData = [
    aboutPageDataResponse?.leader1,
    aboutPageDataResponse?.leader2,
    aboutPageDataResponse?.leader3,
  ]
    .filter(Boolean)
    .map((user) => ({
