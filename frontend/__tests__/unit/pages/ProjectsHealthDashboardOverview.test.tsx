...
describe('ProjectsDashboardOverviewPage', () => {
  ...
  test('renders project health stats', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText('Healthy Projects').length).toBeGreaterThan(0)
      expect(screen.getAllByText('Projects Needing Attention').length).toBeGreaterThan(0)
      expect(screen.getAllByText('Unhealthy Projects').length).toBeGreaterThan(0)
