...
jest.mock('react-apexcharts', () => {
  return function MockChart(props: { options: unknown; series: unknown; height: string | number; type: string }) {
    const mockSeries = props.series as Array<{ name: string; data: Array<{ x: string; y: number; date: string }> }>
    const mockOptions = props.options as Record<string, unknown>

    if (mockOptions.tooltip && typeof mockOptions.tooltip === 'object') {
      const tooltip = mockOptions.tooltip as { custom?: Function }
      if (tooltip.custom) {
        // Test with valid data
        if (mockSeries[0]?.data.length > 0) {
          const result = tooltip.custom({ seriesIndex: 0, dataPointIndex: 0, w: { config: { series: mockSeries } } })
        ...
        tooltip.custom({ seriesIndex: 0, dataPointIndex: 999, w: { config: { series: mockSeries } } })
      }
    }
...
describe('ContributionHeatmap', () => {
  ...
  describe('Rendering & Props', () => {
    it('renders with minimal and all optional props', async () => {
      ...
      rerender(<ThemeProvider attribute="class"><ContributionHeatmap {...defaultProps} title="Activity" unit="commit" /></ThemeProvider>)
    ...
    it('applies custom unit and handles undefined title', () => {
      renderWithTheme(<ContributionHeatmap {...defaultProps} unit="pull request" title={undefined} />)
  ...
  describe('Data Processing & Edge Cases', () => {
    it('handles empty, zero, and high values', () => {
      ...
      testCases.forEach(({ data }) => {
        const { unmount } = renderWithTheme(<ContributionHeatmap {...defaultProps} contributionData={data} />)
        expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
        unmount()
      })
    })
    ...
    it('handles various date ranges', () => {
      ...
      ranges.forEach(({ start, end, data }) => {
        const { unmount } = renderWithTheme(<ContributionHeatmap contributionData={data} startDate={start} endDate={end} />)
        expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
        unmount()
      })
    })
  ...
  describe('Theme & Styling', () => {
    it('renders in light and dark mode with correct classes', () => {
      const { rerender } = renderWithTheme(<ContributionHeatmap {...defaultProps} title="Light" />, 'light')
      ...
      rerender(<ThemeProvider attribute="class"><ContributionHeatmap {...defaultProps} title="Dark" /></ThemeProvider>)
  ...
  describe('Content & Accessibility', () => {
    it('renders title with correct styling and semantic HTML', () => {
      const { container } = renderWithTheme(<ContributionHeatmap {...defaultProps} title="Activity" />)
  ...
  describe('Chart Configuration & Performance', () => {
    ...
    it('re-renders correctly when props change', () => {
      const { rerender } = renderWithTheme(<ContributionHeatmap {...defaultProps} />)
      const newProps = { contributionData: { '2024-02-01': 10 }, startDate: '2024-02-01', endDate: '2024-02-28' }
      rerender(<ThemeProvider attribute="class"><ContributionHeatmap {...newProps} /></ThemeProvider>)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
  ...
  describe('Tooltip Behavior', () => {
    ...
    it('handles singular and plural unit labels in tooltip', () => {
      ...
      rerender(<ThemeProvider attribute="class"><ContributionHeatmap contributionData={singleData} startDate="2024-01-01" endDate="2024-01-31" unit="commit" /></ThemeProvider>)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
    ...
    it('tooltip respects theme colors', () => {
      ...
      const { useTheme } = require('next-themes')
      useTheme.mockReturnValue({ theme: 'dark', setTheme: jest.fn() })
      rerender(<ThemeProvider attribute="class"><ContributionHeatmap {...defaultProps} /></ThemeProvider>)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })

    it('handles missing data in tooltip gracefully', () => {
      const { container } = renderWithTheme(<ContributionHeatmap contributionData={{}} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
  ...
  describe('Week Number Calculation', () => {
    it('correctly calculates week numbers starting from Monday', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={data} startDate="2024-01-01" endDate="2024-01-14" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
    ...
    it('handles week transitions correctly', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={data} startDate="2024-01-07" endDate="2024-01-14" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
  })
  ...
  describe('Color Scale Logic', () => {
    it('applies correct color ranges for different activity levels', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={activityData} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
    ...
    it('handles boundary values in color ranges', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={boundaryData} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
  })
  ...
  describe('Date Range Edge Cases', () => {
    it('handles leap year dates', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={leapYearData} startDate="2024-02-28" endDate="2024-03-01" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
    ...
    it('handles year boundaries correctly', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={yearBoundaryData} startDate="2023-12-30" endDate="2024-01-02" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
    ...
    it('handles dates in reverse chronological order in data object', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={reversedData} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
  })
  ...
  describe('Extreme Values & Data Quality', () => {
    it('handles negative contribution values gracefully', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={negativeData} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
    ...
    it('handles floating point contribution values', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={floatData} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
    ...
    it('handles extremely large contribution counts', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={extremeData} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
  })
  ...
  describe('Component State & Lifecycle', () => {
    it('maintains consistent rendering across multiple updates', () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        rerender(<ThemeProvider attribute="class"><ContributionHeatmap contributionData={newData} startDate="2024-01-01" endDate="2024-01-31" /></ThemeProvider>)
      }
  ...
  describe('String Formatting & Localization', () => {
    it('formats date strings correctly in different formats', () => {
      ...
      renderWithTheme(<ContributionHeatmap contributionData={dateFormatData} startDate="2024-01-01" endDate="2024-01-31" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })

    it('handles custom unit strings with special characters', () => {
      const { unmount } = renderWithTheme(<ContributionHeatmap {...defaultProps} unit="pull-request" />)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
      unmount()
      ...
      rerender(<ThemeProvider attribute="class"><ContributionHeatmap {...defaultProps} unit="contribution/day" /></ThemeProvider>)
      expect(screen.getByTestId('mock-heatmap-chart')).toBeInTheDocument()
    })
  })
  ...
  describe('Integration & Real-world Scenarios', () => {
    it('renders complete heatmap with realistic GitHub contribution data', () => {
      const githubLikeData: Record<string, number> = {
        '2024-01-01': 5, '2024-01-03': 12, '2024-01-05': 3,
        '2024-01-08': 8, '2024-01-10': 15, '2024-01-12': 7,
        '2024-01-15': 20, '2024-01-18': 4, '2024-01-20': 9,
        '2024-01-22': 11, '2024-01-25': 6, '2024-01-28': 13,
      }
      renderWithTheme(
        <ContributionHeatmap 
