...
describe('ChapterMap', () => {
  ...
  describe('Interactive Overlay', () => {
    ...
    it('renders unlock icon before the button text', () => {
      const { getByText, getByTestId } = render(<ChapterMap {...defaultProps} />)
      const overlay = getByText('Unlock map').closest('button')

      // assuming icon is inside the button and has data-testid="unlock-icon"
      const icon = within(overlay!).getByTestId('unlock-icon')
