# Shared Terraform image pin helpers (source of truth: docker/terraform/Dockerfile).

.PHONY: check-terraform-version

# Full image reference including digest, e.g. hashicorp/terraform:1.15.8@sha256:...
TERRAFORM_IMAGE := $(shell sed -nE 's/^FROM[[:space:]]+([^[:space:]]+).*/\1/p' docker/terraform/Dockerfile | head -1)
...
check-terraform-version: ## Verify required_version matches the pinned Terraform image
	...
	if [ -z "$$tag" ]; then \
		echo "Error: could not parse hashicorp/terraform tag from docker/terraform/Dockerfile" >&2; \
		exit 1; \
	fi; \
